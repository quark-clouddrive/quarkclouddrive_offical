...
quarkclouddrive login

# 授权码登录（非交互式）
quarkclouddrive login --token <agent_auth_code>
```
...
> **agent 调用流程**：
> 1. 调用 `quarkclouddrive login`，命令会自动打开浏览器并等待用户授权。
> 2. 授权完成后命令自动返回登录结果。
> 3. 登录成功后，执行后续命令。

### 登录成功后的引导规则

当 `login` 返回 `code: 0` 时，agent **必须**检查以下任一条件是否满足（不依赖 agent 自行判断"是否有未完成任务"，而用确定性标志触发）：
...
反之，**仅当**本次 `login` 是某个原命令执行过程中的自动重试（即 login 紧跟在原命令之后、为完成该操作而触发），登录成功后应**直接继续执行该原命令**，**无需输出**上方欢迎文案。
...
4. agent 收到用户粘贴的授权码后，使用 `quarkclouddrive login --token <授权码>` 完成登录。
...
>
> **话术要求**：在自动登录失败时，agent 必须用明确、引导性的语言告诉用户「把授权码粘贴到对话框发给我」，禁止使用让用户自行在终端执行 `quarkclouddrive login --token` 的表述。
>
...
| `logout` | 仅卸载时由 `uninstall.sh` 内部调用 | 通过 access_token 直接、静默地撤销本机授权，随后删除当前 agent 的配置目录 `~/.quarkclouddrive/<agentId>/` | 是 |
...
`uninstall.sh` 会调用 `logout` 在服务端撤销本设备授权，并删除当前 agent 的配置目录 `~/.quarkclouddrive/<agentId>/`（含登录态 / access_token / 账号信息）。

```bash
bash uninstall.sh
```

> ⚠️ `~/.quarkclouddrive/`（CLI 二进制、各 agent 配置）与全局命令注册（符号链接 / PATH）均为多 agent 共享资源。为避免影响其他 agent，`uninstall.sh` **不会**删除整个 `~/.quarkclouddrive/` 目录，也**不会**移除全局命令 `quarkclouddrive` 及 shell 配置中的 PATH 条目；仅删除当前 agent 的配置目录。

> **禁止**跳过 `uninstall.sh` 直接用 `rm -rf` 删除，否则服务端授权记录不会被撤销。

### Step 2：删除 skill 目录（移除 agent skill 文件）

`uninstall.sh` 只撤销当前 agent 授权并清除其配置目录（`~/.quarkclouddrive/<agentId>/`），**不会**删除共享的 CLI 安装目录（`~/.quarkclouddrive/`）与全局命令注册，也**不会**删除 agent skill 目录中的 skill 文件（`SKILL.md`、`references/` 等）。`uninstall.sh` 执行成功后，agent 还需删除本 skill 所在目录，将 skill 从 agent 环境中完全移除。
